...
int main(int argc, char **argv) {
  ros::init(argc, argv, "vri4wd_rviz_initial2d");
  ros::NodeHandle node;
  pub = node.advertise<geometry_msgs::PoseStamped>("goal_2d", 0);
  pub2 = node.advertise<geometry_msgs::PoseStamped>("initial_2d", 0);
  ros::Subscriber sub = node.subscribe("vri4wd_move/goal", 0, handle_goal); //standard "move_base_simple/goal"
  ros::Subscriber sub2 = node.subscribe("initialpose", 0, handle_initial_pose); //standard "initialpose"

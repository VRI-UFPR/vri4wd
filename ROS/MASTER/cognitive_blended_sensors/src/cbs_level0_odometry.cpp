...
void Calc_Left(const std_msgs::Int16& leftCount) {  
  ...
  lastCountL = leftCount.data;
  ROS_INFO("left pulses:[%f]:", distanceLeft); 
}
...
void Calc_Right(const std_msgs::Int16& rightCount) {
  ...
  lastCountR = rightCount.data;
  ROS_INFO("right pulses:[%f]:", distanceRight);  
}
...
void publish_quat() {
  ...
  quatOdom.child_frame_id = "base_footprint"; //standard "base_link"
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "cbs_level0_odometry");
  ros::NodeHandle node;
  ...
  //ros::Subscriber subForRightCounts = node.subscribe("/right_encoder", 100, callback_right_encoder);
	//ros::Subscriber subForLeftCounts = node.subscribe("/left_encoder", 100, callback_left_encoder);
